...
function init() {
    ...
    timeElement.oninput = function () {
        draw(log.timeline[timeElement.value]);
    }
...
function tick() {
    ...
    if (time+1 < log.timeline.length)
        ...
        pods1 = pods0;
    setOutputs(log.output_timeline[time+1]);
    draw(morph(pods0, pods1, timeInFrame / ticksInFrame));
    timeInFrame++;
...
function drawPod(ctx, pod) {
    ...
    ctx.moveTo(pod.pos.x, pod.pos.y);
    for (let track of pod.tracks) {
